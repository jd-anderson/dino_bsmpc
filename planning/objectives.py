...
def create_objective_fn(alpha, base, mode="last", use_bisim=False, bisim_weight=1.0):
    """
    ...
    def objective_fn_last_bisim(z_obs_pred, z_obs_tgt):
        ...
        # Get model's encode_bisim function
        encode_bisim = next(iter(z_obs_pred["visual"].values())).module.wm.encode_bisim
        ...
        # Calculate L2 distance in bisimulation space
        bisim_loss = torch.norm(bisim_pred - bisim_tgt, dim=-1)
    ...
    def objective_fn_all_bisim(z_obs_pred, z_obs_tgt):
        ...
        # Calculate L2 distance in bisimulation space for each timestep
        bisim_dist = torch.norm(bisim_pred - bisim_tgt, dim=-1)
